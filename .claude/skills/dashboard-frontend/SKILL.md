...
description: Reactダッシュボード（dashboard/）のアーキテクチャ設計判断。タブ切り替えUI、KEV/OSV/JVN共通パーツ（VulnPanelParts）、DepscanPanelの集約・オーナーフィルター、CSSカスケードレイヤーの注意点、環境表示の日本語化を扱う。dashboard/配下のコンポーネント変更時に読む。
---
...
`jvn.ts`/`depscan.ts`/`depsops.ts`/`crawlerLogs.ts`/`auth.ts` に分割し、`client.ts`自体は
バーレル（re-export）として残している。既存の`import { xxx } from '../api/client'`は
全て動き続ける。新しいAPI呼び出し関数を追加する場合は対応ドメインのファイルに追加すること。
