...
from app.cron_depscan import (  # noqa: E402
    _build_findings,
...
from app.cron_osv import _query_versions_batch  # noqa: E402
from app.github_client import get_file_content, get_repo_tree, list_target_repos  # noqa: E402
from app.models import DependencyFinding  # noqa: E402
from app.notifications import notify_dependency_findings  # noqa: E402
...
class TestListTargetRepos:
    def test_excludes_fork_and_archived(self):
        ...
        with patch("app.github_client.httpx.Client", return_value=mock_client):
...
# ──────────────────────────────────────────────────────────────
# app.cron_osv._query_versions_batch
# ──────────────────────────────────────────────────────────────
...
class TestDiscoverManifests:
    def test_filters_by_known_filenames(self):
        with patch(
            "app.cron_depscan.get_repo_tree",
...
class TestCollectDependencies:
    def test_aggregates_across_repos(self):
        ...
        with patch("app.cron_depscan.list_target_repos", return_value=repos), \
             patch("app.cron_depscan._discover_manifests", return_value=["requirements.txt"]), \
             patch("app.cron_depscan.get_file_content", return_value="fastapi==0.115.6\n"):
    ...
    def test_fetch_failure_skips_file(self):
        ...
        with patch("app.cron_depscan.list_target_repos", return_value=repos), \
             patch("app.cron_depscan._discover_manifests", return_value=["requirements.txt"]), \
             patch(
                 "app.cron_depscan.get_file_content",
...
class TestBuildFindings:
    def test_builds_records_from_hits(self):
        ...
        with patch(
            "app.cron_depscan._query_versions_batch",
            return_value={("PyPI", "cryptography", "3.4.7"): ["GHSA-crypto-001"]},
        ), patch("app.cron_depscan._fetch_vuln_by_id", return_value=vuln):
    ...
    def test_no_hits_returns_empty(self):
        with patch("app.cron_depscan._query_versions_batch", return_value={}):
    ...
    def test_vuln_fetch_failure_skipped(self):
        ...
        with patch(
            ...
            "app.cron_depscan._fetch_vuln_by_id",
            side_effect=httpx.HTTPStatusError("500", request=MagicMock(), response=MagicMock()),
        ):
...
class TestFetchAndScanDependencies:
    def test_success_path(self, db_session):
        with patch(
            "app.cron_depscan._collect_dependencies",
            return_value=({("PyPI", "pkg", "1.0.0"): [("u/r", "requirements.txt")]}, 1),
        ), patch(
            "app.cron_depscan._build_findings",
            return_value=[{
        ...
        ), patch("app.cron_depscan.SessionLocal", return_value=db_session), \
           patch("app.cron_depscan.notify_dependency_findings") as mock_notify:
    ...
    def test_error_path_logs_and_notifies(self, db_session):
        with patch(
           ...
           patch("app.cron_depscan.notify_depscan_crawl_error") as mock_notify_error:
            import pytest
...
# ──────────────────────────────────────────────────────────────
# app.notifications.notify_dependency_findings
# ──────────────────────────────────────────────────────────────
...
class TestNotifyDependencyFindings:
    ...
    def test_skips_when_no_webhook(self):
        with patch("app.notifications.settings.SLACK_WEBHOOK_URL", ""):
    ...
    def test_skips_when_empty(self):
        with patch("app.notifications.settings.SLACK_WEBHOOK_URL", "https://hooks.slack.com/x"):
            with patch("app.notifications._send_slack") as mock_send:
