"""app.depsops.user_runner.run_dependabot_ops_for_user（Issue #227）のテスト。"""
...
class TestRunDependabotOpsForUser:
    def test_success_path_merges_and_notifies_only_the_user(self, db_session):
        ...
        with patch("app.depsops.user_runner.list_target_repos", return_value=repos), \
             ...
             patch("app.depsops.user_runner.notify_dependabot_ops") as mock_notify:
    ...
    def test_records_pr_logs(self, db_session):
        ...
        with patch("app.depsops.user_runner.list_target_repos", return_value=repos), \
             ...
             patch("app.depsops.user_runner.notify_dependabot_ops"):
            run_dependabot_ops_for_user("octocat", "gho_token", _WEBHOOK)
    ...
    def test_outer_failure_does_not_raise(self, db_session):
        with patch(
            "app.depsops.user_runner.list_target_repos", side_effect=RuntimeError("API down"),
        ):
