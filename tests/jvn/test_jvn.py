...
def test_fetch_page_returns_none_on_http_error(monkeypatch):
    ...
    from app.jvn.crawler import _fetch_page

    def mock_get(*args, **kwargs):
        raise httpx.ConnectError("connection refused")

    monkeypatch.setattr("app.jvn.crawler.httpx.Client", lambda **kw: MagicMock(
...
def test_fetch_all_entries_returns_empty_on_none(monkeypatch):
    ...
    monkeypatch.setattr("app.jvn.crawler._fetch_page", lambda *a: None)

    from app.jvn.crawler import _fetch_all_entries
...
def test_fetch_page_success(monkeypatch):
    ...
    monkeypatch.setattr("app.jvn.crawler.httpx.Client", lambda **kw: mock_client)
...
def test_fetch_all_entries_with_items(monkeypatch):
    ...
    monkeypatch.setattr("app.jvn.crawler._fetch_page", lambda *a: root)
...
def test_fetch_all_entries_pagination(monkeypatch):
    ...
    monkeypatch.setattr("app.jvn.crawler._fetch_page", mock_fetch_page)
    monkeypatch.setattr("app.jvn.crawler.time.sleep", lambda s: None)
...
def test_upsert_jvn_periodic_commit(db_session):
    ...
    with patch("app.jvn.crawler._COMMIT_EVERY", 1):
        inserted, updated = _upsert_jvn(db_session, entries)
