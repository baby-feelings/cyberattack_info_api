...
| 機能 | 説明 |
|------|------|
| **CISA KEV 自動クローラー** | 毎日 AM 4:00 (JST) に CISA KEV フィードを取得・Upsert |
| **OSV 自動クローラー** | 毎日 AM 5:00 (JST) に OSV API から主要パッケージの脆弱性を取得・Upsert |
...
| **OSV 脆弱性 API** | OSV データの一覧・エコシステム/重要度別統計 |
...
9 エコシステム（PyPI / npm / Go / Maven / RubyGems / NuGet / crates.io / Packagist / Hex）の主要パッケージの脆弱性一覧を取得します。
...
| `OSV_DAYS` | - | OSV 取得対象の直近日数（デフォルト: `90`） |
