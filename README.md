...
| **JVN 自動クローラー** | 毎日 JST 06:05 に MyJVN API から国内脆弱性を取得・Upsert |
...
| **React ダッシュボード** | CISA KEV・OSV・JVN の 3 セクション構成（Vercel デプロイ） |
...
├── dashboard/               # Vercel デプロイの React ダッシュボード（KEV・OSV・JVN）
├── .github/
│   └── workflows/
│       ├── ci.yml           # CI: lint + type check + test (PR 時に自動実行)
│       ├── deploy.yml       # CD: Render デプロイ (main マージ時に自動実行)
│       └── daily-crawl.yml  # 毎日クロール (KEV 04:05 / OSV 05:05 / JVN 06:05 JST)
...
> **Note:** APScheduler（アプリ内スケジューラ）も UTC 19:00 / 20:00 / 21:00 で動作していますが、  
