...
| **レスポンス形式** | JSON |
| **データソース** | CISA KEV Catalog（毎日 JST 04:05 自動更新）／OSV API（毎日 JST 05:05 自動更新） |
...
### スキル 7: クローラーの実行ログを確認する

**用途:** KEV / OSV クローラーが正常に動作しているか、最新の実行結果（件数・所要時間・エラー）を確認する。
...
# KEV のみ絞り込み
curl -s -H "X-API-KEY: $CYBERATTACK_API_KEY" \
  "https://cyberattack-info-api.onrender.com/api/crawler-logs?crawler_type=KEV"
...
- name: Check crawler health
  run: |
    RESULT=$(curl -s -H "X-API-KEY: ${{ secrets.CYBERATTACK_API_KEY }}" \
      "https://cyberattack-info-api.onrender.com/api/crawler-logs?limit=2&crawler_type=KEV")
    STATUS=$(echo "$RESULT" | python -c "import sys,json; d=json.load(sys.stdin); print(d[0]['status'] if d else 'no_log')")
    echo "最新 KEV クロール: $STATUS"
...
|-----------|-----|------|
| `id` | int | ログ ID |
| `crawler_type` | string | クローラー種別（`KEV` / `OSV`） |
