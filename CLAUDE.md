...
├── schemas.py         # Pydantic スキーマ（VulnerabilityOut・OsvVulnerabilityOut・CrawlerLogOut 等）
...
└── daily-crawl.yml  # 毎日クロール: KEV(UTC 19:05) / OSV(UTC 20:05) を GitHub Actions から叩く
```
...
`.github/workflows/daily-crawl.yml` で GitHub Actions cron が毎日 `/admin/crawl`・`/admin/osv-crawl` を叩いて補完している。  
...
- `workflow_dispatch` で手動実行も可能（`target: kev / osv / both`）
