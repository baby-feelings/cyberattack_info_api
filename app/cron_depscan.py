...
from app.cron_osv import _fetch_vuln_by_id, _parse_severity, _query_versions_batch
from app.database import SessionLocal
from app.dependency_parsers import LOCKFILE_FILENAMES, parse_manifest
from app.github_client import get_file_content, get_repo_tree, list_target_repos
from app.models import DependencyFinding
from app.notifications import notify_dependency_findings, notify_depscan_crawl_error
...
def _resolve_stale_findings(db: Session, current_keys: set[FindingKey]) -> int:
    """今回のスキャンで検知されなくなった未解決 Finding を解決済みにする。"""
    resolved = 0
    now = now_utc()
    open_findings = (
        db.query(DependencyFinding)
        .filter(DependencyFinding.resolved_at.is_(None))
        .all()
    )
